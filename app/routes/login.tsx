...
import { createPlayerSession, login, register } from '~/utils/session.server';
...
export const action: ActionFunction = async ({ request }) => {
  ...
  switch (loginType) {
    case 'login': {
      ...
      return createPlayerSession(player.id, redirectTo);
    }
    case 'register': {
      const userExists = await db.player.findFirst({
        where: { username },
      });
      if (userExists) {
        return badRequest({
          fields,
          formError: `Player with username ${username} already exists`,
        });
      }
